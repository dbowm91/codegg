...
- **Pass 8 — Generation authority**: `generation_map: Arc<Mutex<HashMap<String, u64>>>` is the single source of truth for per-client generation. `LspService::generation_for_key(key)` and `set_generation(key, gen)` are the public accessors. The first publish sets generation `1`; the restart coordinator bumps it after a successful reinit + replay. Exit events whose `event.generation != current_generation` are silently dropped. `LspOperationalHealthSnapshot` carries `generation: u64` from the map, constructible without a live client.
- **Pass 9 — Shutdown integration**: `shutdown_all()` is quiescent with a bounded global deadline (`Instant::now() + SHUTDOWN_GLOBAL_TIMEOUT` = 6s). The sequence transitions to `ShuttingDown`, drains init slots, drains active tasks, signals cooperative cancellation, awaits completion receivers via `await_init_task_completions`, aborts stragglers, drains ready clients, and transitions to `Stopped`. `shutdown_inner()` is the internal implementation; `shutdown_all()` is the public entry point. The lifecycle is broadcast on a `watch` channel so late subscribers do not lose wakeups at the `ShuttingDown → Stopped` transition.
- **Pass 10 — RwLock deadlock fix**: Lock ordering is documented on `LspService`: clients map lock before client-level lock. Non-mutating service methods use `clients.read().await`; write guards are reserved for slot election/publication and shutdown drain. No guard is held across client I/O. `close_file` and `save_file` use deterministic O(1) ownership lookup via the `document_owners` map (URI → client key) rather than iterating cloned handles.
- **Pass 11 — Descriptor-first init + restart config**: `LspClientDescriptor` persists the per-client launch spec (key, server_id, root, launch_spec, init opts, workspace config, readiness/restart policy, seed file). `LspClientDescriptor::from_profile()` resolves with explicit `user > profile > server-definition` priority. User-configurable restart via `[lsp.<server>.restart]` TOML section overrides profile defaults; `LspRestartPolicyConfig` (in `crates/egglsp/src/config.rs`, mirrored in `crates/codegg-config/src/schema.rs`) has optional fields: `mode`, `max_attempts`, `initial_backoff_ms`, `max_backoff_ms`, `reset_after_healthy_secs`. The restart coordinator reads the descriptor directly — no language detection, no `src/lib.rs` synthesis.
- **Pass 12 — Diagnostic generation binding**: `DiagnosticCacheEntry` carries `server_generation: u64` (0 is the "never assigned" sentinel) and `post_restart: bool` (monotonically sticky). `LspDiagnosticSnapshot` exposes both fields. On restart, `mark_diagnostics_stale_for_key(key)` re-keys retained diagnostics to `current - 1` so the freshness classifier returns `LspDiagnosticFreshness::Stale` until the new server emits its first push. The root `SemanticContextCollector` propagates these fields to `SemanticDiagnosticEvidence`.
- **Pass 13 — Readiness semantics**: `LspReadinessPolicy` is a 4-variant enum: `InitializedIsReady`, `WaitForDiagnosticsOrTimeout { timeout }`, `WaitForProgressEndOrTimeout { timeout }`, `WarmupDelay { duration }`. `LspService::wait_for_readiness(key, policy)` returns `ReadinessResult::Ready { elapsed }` or `ReadinessResult::Degraded { reason, elapsed }`. `LspClient` tracks `ProgressState` (active `$/progress` tokens + last progress timestamp) and exposes `progress_snapshot()`, `wait_for_progress_end(timeout)`, `wait_for_first_diagnostics(timeout)`, and `operational_summary()`.
- **Pass 14 — Exit metadata persistence**: `LspProcessExitEvent` carries generation, status, signal, expected flag, and stderr tail. Exit metadata is persisted in the service's operational state map so health snapshots remain available even when no live client exists (during `RestartScheduled`, `Restarting`, `Failed`, `Stopped`). The `expected` flag is derived from `LspProcessIntent` at exit time — transport state never determines expectedness.
- **Pass 15 — Real-server harness**: `crates/egglsp/tests/real_server_smoke.rs` exercises rust-analyzer and pyright/basedpyright against the production launcher, frame parser, and request routing. Gated behind `lsp-real-server-tests` feature. `assert_required_checks(report)` fails the test on `Required` regressions. `.github/workflows/lsp-real-server.yml` pins toolchains and runs one server per matrix job.
