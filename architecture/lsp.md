...
- **Packaging**: `cargo package -p egglsp` succeeds with all target paths contained inside the package. The `egglsp-test-server` binary is now a self-contained package-local wrapper. A shared `egglsp-test-server-support` crate (with `publish = false`) provides the scenario engine implementation used by both the root `codegg-lsp-test-server` binary and the `egglsp` package-local binary.
