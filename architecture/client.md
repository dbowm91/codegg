...
Both the remote TUI client (`src/client/`) and the local `SocketCoreClient` connect through the user-scoped singleton daemon. The canonical entry point is `connect_or_start_daemon` (`src/core/instance.rs`), which performs a complete bounded handshake/identity probe before reusing a daemon or auto-starting one. A peer EOF fails outstanding socket requests rather than leaving them pending, and reconnects establish a fresh handshake. The `SnapshotDaemon` request/response surfaces `daemon_id`, `uptime_secs`, `active_sessions`, and `connected_clients`; the `daemon status` CLI prints `generation` and `started_at` from the on-disk metadata file.
