...
An optional LSP enrichment pass (`--enrich`) executes bounded, read-only `securityContext` requests for escalated targets via the `SecurityContextExecutor` trait, then reruns synthesis with enriched CallPath/Diagnostic/TruncationNotice evidence. The `LspSecurityContextExecutor` adapter (in `src/security/lsp_executor.rs`) wraps `LspTool` and validates requests via `validate_security_context_request()`. The command-level function `run_security_review_command_with_executor()` accepts an optional executor. Enrichment is opt-in, fail-soft, and never mutates files.
