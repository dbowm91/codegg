...
When the resolved agent has `runtime_kind = security_review`, `DefaultTurnRuntime` runs the deterministic workflow before starting the ordinary agent loop. It injects only a bounded `SecurityEvidenceBundle` containing target locations, check summaries, coverage, evidence gaps, and a SHA-256 fingerprint into the canonical prompt. The full workflow output remains host-side. The same prepared targets drive task-aware `securityContext` LSP input when an LSP service is available. This path does not create a security-specific scheduler or tool registry; the existing AgentLoop, permission surface, cancellation, and scheduler remain authoritative. Model-produced findings are accepted only when they reference an in-scope target with evidence and reasoning; marker-only material remains a review prompt.
