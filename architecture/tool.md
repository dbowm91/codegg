...
The `/security-review` TUI command exposes the workflow. The command handler is testable via `parse_security_review_args()` and `run_security_review_command()` in `src/security/workflow/report.rs`. Flags: `--changed` (shorthand for `--base HEAD`), `--base <ref>`, `--json`, `--prompts-only`, `--findings-only`, `--no-content`, `--no-filename`, `--max-findings N`, `--max-prompts N`.
