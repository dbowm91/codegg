...
CodeGG is a high-performance AI coding agent built in Rust, designed for terminal-based interaction with deep IDE and LSP integration. This document provides a bird's eye view of the entire system and serves as an index to detailed architecture documents for each component.

## Deployment Topology

One `codegg` executable per target. The daemon is a user-scoped singleton discovered and started by that executable; the TUI and daemon share the same business-logic libraries and invocation contracts. A daemon/TUI split does not produce a material deployment improvement without adding packaging or ownership complexity.
...
| Command Outcome | Records actual executor used and command results for downstream attribution | `command_outcome.rs` | [command_outcome.md](command_outcome.md) |
...
| Git Mutations | Typed Git mutations with state deltas, snapshot/delta capture, RunStore persistence | `git_mutations.rs`, `git_mutations_ops.rs` | [git_mutations.md](git_mutations.md) |
| Git Network | Network operations (fetch/pull/push/remote), env hardening, URL credential redaction | `git_network_ops.rs`, `git_network_policy.rs` | [git_network.md](git_network.md) |
| Git Recovery | In-progress operation detection, continue/abort/skip with cross-operation misuse protection | `git_recovery.rs`, `git_run_store.rs` | [git_recovery.md](git_recovery.md) |
...
| Job Dispatcher | Bridges durable jobs to existing executors (SubAgent, ManagedArgv, Test) | `job_dispatcher.rs`, `job_recovery.rs` | [job_dispatcher.md](job_dispatcher.md) |
| Managed Process | Managed process lifecycle with process-group cleanup, timeout, cancellation, descendant tracking | `managed_process.rs` | [managed_process.md](managed_process.md) |
| Session | SQLite session storage, message history, 35 migrations, analytics, checkpointing | `session/` (codegg-core) | [session.md](session.md) |
...
| Crate | Purpose | Key Files | Docs |
|-------|---------|-----------|------|
| `codegg-core` | Domain types: bus, error, goal, memory, migration, run_store, session, storage, snapshot, worktree, workspace, workspace_services, task_state, model_profile, resilience, protocol_conversions | `lib.rs`, `bus/`, `jobs/`, `session/`, `storage/` | [codegg_core.md](codegg_core.md) |
| `codegg-config` | Configuration schema, paths, loading, validation, file watching | `schema.rs`, `paths.rs`, `watcher.rs` | [config.md](config.md) |
| `codegg-protocol` | CoreRequest, CoreResponse, CoreEvent, TuiMessage, UiNode, UiEffect, PluginManifestDto | `core.rs`, `tui.rs` | [protocol.md](protocol.md) |
| `codegg-providers` | LLM provider implementations, auth types, CircuitBreaker | `provider/mod.rs`, `auth/`, `circuit.rs` | [provider.md](provider.md) |
| `codegg-git` | Typed Git operation model, argv parser, and risk classification (47 operation variants, 11 risk classes) | `lib.rs` | [native_crates.md](native_crates.md) |
| `egglsp` | LSP client/service/operations (authoritative implementation) | `service.rs`, `client.rs`, `operations.rs`, `server.rs` | [lsp.md](lsp.md) |
| `egggit` | Read-only git facts: status (v2 rich structured), diff, changed files, log, blame, refs, worktree | `status_v2/`, `diff/`, `log/`, `blame/`, `refs/`, `worktree/` | [git.md](git.md) |
| `eggsentry` | Security scanning — secrets, commands, dependencies, unsafe code | `scanner.rs`, `command.rs`, `finding.rs`, `profile.rs` | [security.md](security.md) |
| `eggcontext` | Token counting and context utilities (tiktoken-based) | `lib.rs` | [context-ledger.md](context-ledger.md) |
...
| Module | Purpose | Key Files | Docs |
|--------|---------|-----------|------|
| Context | Context budgeting, packing, and projection for LLM conversation history | `context/` | [context.md](context.md) |
...
## Navigation — Deep-Dive Index

Each module has a dedicated architecture document. Use the links below to jump to the detailed specification.

### Agent and Execution
- [Agent Loop](agent.md) — Main execution cycle, compaction, routing, multi-agent teams, asset context/snapshot/refresh
- [ACP](acp.md) — Agent Client Protocol v1 stdio adapter
- [Command Intent](command_intent.md) — Command classification, risk assessment, execution capabilities (stage 1)
- [Command Planner](command_planner.md) — Backend mapping, permission generation, projection policy (stage 2)
- [Command Routing](command_routing.md) — Concrete subsystem dispatch (stage 3)
- [Command Outcome](command_outcome.md) — Actual executor attribution and result recording
...
- [Git Mutations](git_mutations.md) — Typed mutation execution with snapshot/delta persistence
- [Git Network](git_network.md) — Network operations, env hardening, credential redaction
- [Git Recovery](git_recovery.md) — In-progress operation detection, continue/abort/skip
...
- [Job Dispatcher](job_dispatcher.md) — Bridges durable jobs to executors
- [Managed Process](managed_process.md) — Process lifecycle, group cleanup, descendant tracking
...
- [Eggsentry](eggsentry.md) — Deterministic security scanning
- [Crypto](crypto.md) — AES-256-GCM encryption
- [Auth](auth.md) — Authentication and credentials

### Support
- [Context](context.md) — Context budgeting, packing, and projection
...
│   ├── context/                # Context budgeting, packing, projection
...
├── tests/                      # Integration tests (163 files)
