...
| `SourcePreviewDialog` | Read-only source preview dialog at `src/tui/components/dialogs/source_preview.rs`. Root-scoped; used for security review finding navigation. | `src/tui/components/dialogs/source_preview.rs` |
...
| Testable command functions | `SecurityReviewCommandArgs` (includes `open_panel_on_complete: bool` for `--panel`), `parse_security_review_args()`, `run_security_review_command()` — testable CLI argument parsing and command execution. | `src/security/workflow/report.rs` |
...
| `SecurityReviewHunkRef` | Compact hunk context for TUI display, carrying `file_path`, `old_start`, `old_lines`, `new_start`, `new_lines`, `header`, and `lines: Vec<SecurityReviewHunkLine>`. Built from parsed `ChangedHunk`s by `build_hunk_refs_from_changed_hunks()` in `report.rs`. | `src/security/workflow/types.rs:146` |
| `SecurityReviewHunkLine` | Single line within a hunk with `old_line: Option<u32>`, `new_line: Option<u32>`, `kind: SecurityReviewHunkLineKind`, `text: String`. | `src/security/workflow/types.rs:158` |
...
| `SecurityReviewPanelItem.hunk` | `Option<SecurityReviewHunkRef>` — findings/prompts are matched to hunks by file_path + line in range via `hunk_index` in `project_receipt_to_panel_items()`. | `src/security/workflow/receipt.rs:85` |
