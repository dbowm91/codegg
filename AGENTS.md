...
- **Plugin management UX (Phase 12)**: `src/plugin/management.rs` provides `PluginManager` with `list()`, `info()`, `enable()`, `disable()`, `doctor()`, `uninstall()` methods. `PluginManagementView` is the flattened view struct. `resolve_plugin_selector()` resolves by exact id → exact name → unique prefix. `src/plugin/management_ui.rs` provides `plugins_table()`, `plugin_info_node()`, `doctor_report_node()` returning portable `UiNode`. `src/tui/commands/plugin_management.rs` has 7 handlers: `show_plugins`, `show_plugin_info`, `enable_plugin`, `disable_plugin`, `doctor_plugin`, `remove_plugin`, `install_plugin`. Commands registered: `/plugins` (aliases `/plugin-list`, `/plugin-ls`), `/plugin-info`, `/plugin-enable`, `/plugin-disable`, `/plugin-doctor`, `/plugin-remove`, `/plugin-install`. Enable/disable persistence uses a `disabled_plugins.json` file in the plugins dir. `PluginRegistry::set_enabled` now takes `&self` (not `&mut self`) since mutations use interior RwLocks.
