...
| egglsp-test-server bin target | `crates/egglsp-test-server/src/main.rs` (built by `egglsp`) | Fake LSP server binary for deterministic integration testing and scenario-engine self-tests. Reads Content-Length framed JSON-RPC, executes scripted scenarios, writes machine-readable transcripts. Root tests use `codegg-lsp-test-server` (via `CARGO_BIN_EXE_codegg-lsp-test-server`). |
...
| Root composite test count | 21 | `tests/lsp_composite_stdio.rs` — exercises `SemanticContextCollector`, `DiagnosticsCollector`, `LspOperations`, and security context tool orchestration (including call hierarchy error degradation) against fake server via production stack |
...
# LSP integration tests (fake server + production harness, no network)
