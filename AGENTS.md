...
- **Plugin runtime abstraction (Phase 6)**: `src/plugin/runtime/` defines `PluginRuntime` trait (`async fn invoke(PluginInvocation) -> Result<PluginResponse, RuntimeError>`) with `ProcessRuntime` implementation. `RuntimeError` covers Unsupported, Spawn, Timeout, NonZeroExit, InvalidJson, Io. `RuntimeLimits` controls timeout_ms, max_stdout_bytes, max_stderr_bytes. `ProcessRuntimeSpec` converts from both `ProcessCommandSpec` and `PluginRuntimeSpec::Process`. `ProcessRuntime` handles child process spawning, stdin piping, timeout, output capping, and stdout mode parsing (Text/Json/Auto). `PluginResponse` is now unified: the local `PluginResponse` in `service.rs` is removed; `codegg_protocol::plugin::PluginResponse` (with `effects: Vec<UiEffect>`) is the single canonical type re-exported from `plugin/mod.rs`. `PluginDiagnostic` is also unified via re-export from protocol. `PluginRegistry::unregister()` now returns `Option<PluginInfo>`. Duplicate checking is global (all registered plugins, not just enabled). `set_enabled(true)` validates no duplicates will be introduced. `is_enabled_sync` defaults to `false` on lock failure. TUI `start_plugin_command` delegates to `ProcessRuntime` instead of inline process spawning.
