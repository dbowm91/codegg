...
- **Plugin management UX (Phase 12)**: `src/plugin/management.rs` provides `PluginManager` with `list()`, `info()`, `enable()`, `disable()`, `doctor()`, `remove()`, `install_from_path()`, and `uninstall()` methods. `PluginManager` is the single canonical API used by TUI command handlers, wrapping `PluginService` + `PluginRegistry` with builtins. `PluginManager::enable/disable` actually call `PluginRegistry::set_enabled`. `PluginManagementView` is the flattened view struct with `from_info` (registry) constructor — `from_marketplace` was removed; the registry is the source of truth. Selector resolution lives on `PluginRegistry::resolve_plugin_selector` (exact id → exact name → unique prefix). `src/plugin/management_ui.rs` provides `plugins_table()`, `plugin_info_node()`, `doctor_report_node()` returning portable `UiNode`, plus `node_to_lines()` for textual fallback rendering. `src/tui/commands/plugin_management.rs` has 7 handlers (`show_plugins`, `show_plugin_info`, `enable_plugin`, `disable_plugin`, `doctor_plugin`, `remove_plugin`, `install_plugin`) all delegating to `PluginManager`. Commands registered: `/plugins` (aliases `/plugin-list`, `/plugin-ls`), `/plugin-info`, `/plugin-enable`, `/plugin-disable`, `/plugin-doctor`, `/plugin-remove`, `/plugin-install`. TUI handlers render output via the `UiNode` → `node_to_lines` path so all visible output flows through the portable schema. Enable/disable is runtime-only (in-memory `PluginRegistry::set_enabled`); `/plugins` shows a notice. `App::plugin_manager: Option<PluginManager>` exists (initialized via `init_plugin_manager()`); `App::new_for_testing` initializes a non-None manager. `PluginRegistry::set_enabled` takes `&self` (not `&mut self`) since mutations use interior RwLocks. Safety is enforced by `PluginManager::uninstall` calling `install::validate_uninstall_target`; `verify_remove_target_is_safe` was removed (its tests moved to `src/plugin/install.rs::tests`). Doctor checks: registered, manifest_valid, api_version, has_capabilities, runtime_configured, runtime_available (process exec or WASM feature), enable_state, no_duplicate_capabilities, permissions_declared, output_surfaces, install_path, no_error_diagnostics, registry_consistency.
...
- **Plugin UI corrective convergence pass (2026-07-02)** closes the remaining convergence and safety gaps after Phase 11–15. Installer path validation in `src/plugin/install.rs` rewritten: `copy_dir_all` validates destination containment (not source containment) via lexical path components; `extract_plugin_archive` rejects symlinks, hardlinks, and absolute paths before extraction and uses lexical containment checks; `validate_install_source` does lexical `ParentDir`/`RootDir`/`Prefix` rejection BEFORE canonicalizing (so the original user-supplied path is inspected); `uninstall()` calls `validate_uninstall_target(plugin_path, &policy)` and rejects name components (path separators, `..`, absolute paths). New shared helper: `validate_relative_install_path(Path) -> Result<(), String>` used by both `copy_dir_all` and `extract_plugin_archive`. TUI management unifies around `PluginManager` — no more `MarketplaceService`/sidecar `disabled_plugins.toml` in TUI command paths. 23 installer tests added (traversal, symlinks, hardlinks, absolute paths, nested installs) plus 13 management tests. Validation commands: `cargo fmt --check`, `cargo check --workspace --all-features`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo test --workspace --all-features`, plus SDK/example validation.
- **Plugin UI final polish pass (2026-07-02)** follows `plans/plugin_ui_final_polish_pass.md`. `PluginInfo` gains `source: Option<PluginSourceMetadata>` (`install_path`, `original_source_path`, `installed_by: PluginInstallKind` of `Builtin|LocalPath|RegistryLoaded|Unknown`); `PluginManagementView::from_info()` now populates `source_path` from this metadata so `/plugin-info` and `/plugin-doctor` show install paths for installed plugins. `PluginManager::install_from_path()` records `LocalPath` metadata with both the user-supplied source and the install destination. `PluginManager::uninstall()` returns a structured `PluginUninstallResult { view, unregistered, removed_files, install_path, warning }` and follows validate → unregister → delete ordering; builtins never get filesystem removal. `TuiCommand::PluginRemoveFinished` carries `removed_files`, `install_path`, and `warning` fields, and `apply_plugin_remove_finished` renders four distinct messages (files removed / no source path / delete failed / hard error). `/plugin-install` now reports the install path and confirms enable state is runtime-only. `install::validate_local_install_source` is a new helper that accepts absolute and `..` paths when canonicalization resolves to a real directory with `manifest.toml`; `validate_relative_install_path` stays strict for archive members and copy-relative paths. CI: `.github/workflows/ci.yml` adds `plugin-focused` (install/management/registry/TUI/boundary) and `examples` (Rust SDK, Python SDK, three WASM examples) jobs; `scripts/validate_plugin_ui.sh` reproduces them locally. Tests: 5 source metadata + structured uninstall tests in `management.rs`, 4 registry round-trip tests, 6 `validate_local_install_source` tests + 2 regression guards in `install.rs`.
