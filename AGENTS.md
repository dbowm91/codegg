...
- **GitMutating backend**: Safe git mutations (add, commit, stash, checkout, switch, restore, merge, rebase, cherry-pick, revert) route to `GitMutating { tool_name, argv }` → `RouteToManagedProcess`. Dangerous mutations (push, pull, reset --hard, clean -f, branch -D) route to `RawShell`.
...
- **Command planner maps intent to backend**: `plan_execution()` in `src/command_intent/plan.rs` maps classified intents to `ExecutionBackend` (RawShell, ManagedArgv, NativeTool, TestRunner, PythonScript, GitMutating, Reject) with rich struct variants carrying metadata. Re-exports from `src/command_planner.rs`.
...
- **Permission planning**: `CommandPermissionRequest` carries `PermissionDefault` (Allow, Ask, Deny) per capability. `generate_permission_requests()` maps `ExecutionCapability` → permission request with context-aware defaults. `DestructiveFileMutation` → `Deny`; `OutsideWorkspace` → `Deny`; `DependencyInstall` → `Deny`. `GitMutation` → `Allow` for safe subcommands (add, commit, stash push, checkout, switch, restore), `Ask` for all others. `WriteWorkspace` → `Allow` for formatters (Format kind or cargo fmt/prettier/black/isort), `Ask` for other writes. `ReadWorkspace`, `Subprocess`, `EnvAccess`, `ContextPromotion` → `Allow`. `Network` → `Ask`.
- **Command routing resolves to subsystem**: `resolve_routing()` in `src/command_routing.rs` maps planned execution to concrete `RoutingDecision` variants (RouteToTestRunner, RouteToShell, RouteToPythonScripting, RouteToNativeTool, RouteToManagedProcess, Rejected). GitMutating backend routes to RouteToManagedProcess.
