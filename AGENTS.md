...
- **LSP Phase 2 fake server**: `crates/egglsp-test-server/` provides a deterministic scripted LSP server for integration testing. Scenario JSON is delivered via `CODEGG_FAKE_LSP_SCENARIO` env var, transcripts via `CODEGG_FAKE_LSP_TRANSCRIPT`. Cargo exposes the binary to tests via `CARGO_BIN_EXE_egglsp-test-server`, with `EGGLSP_TEST_SERVER` as an override for manual or CI use. Tests exercise real stdio transport through `egglsp::launch::spawn_server` and `LspWriter`. 32 protocol tests (initialization, server requests, apply-edit refusal, interleaved notifications, concurrent out-of-order responses, graceful shutdown, server exit, error responses, diagnostics lifecycle, malformed JSON-RPC (10 variants), malformed framing headers (8 variants), nonzero exit code, dynamic registration, cancel notification, workspace folders) and 12 semantic tests (D1 document lifecycle, D2 hover/definition/references/symbols + concurrent out-of-order semantic requests, D3 call hierarchy, D4 rename + code actions, D5 semantic context composite, D6 security context composite, D7 hunk source context). All 44 integration tests passing.
...
| egglsp-test-server crate | `crates/egglsp-test-server` | Fake LSP server binary for deterministic integration testing and scenario-engine self-tests. Reads Content-Length framed JSON-RPC, executes scripted scenarios, writes machine-readable transcripts. |
...
# LSP integration tests (fake server, no network)
