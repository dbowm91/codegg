...
| Security workflow module | `src/security/workflow.rs` | Types: SecurityReviewTarget, SecurityReviewFinding, SecurityReviewPrompt, SecurityPreflightResult, SecurityReviewOutput. Functions: discover_targets_from_diff, run_preflight_checks, synthesize_findings, select_preset_for_file, should_skip_file, parse_unified_diff_hunks |
...
- **Security review workflow**: `src/security/workflow.rs` provides a structured workflow for defensive code review. Risk markers from `securityContext` are review prompts, not findings. Findings require concrete evidence. The workflow is read-only and never mutates files.
