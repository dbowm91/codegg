...
| `collect_hunk_source_context_for_file()` | Async. Uses `HunkSourceContextPolicy` to decide whether to invoke `hunkSourceContext` for a file's hunks. Returns evidence, summary, and notes. Fail-open. | `src/security/workflow/report.rs:1007` |
| `collect_hunk_source_context_all_files()` | Async. Groups hunks by file, calls `collect_hunk_source_context_for_file` per file, merges evidence/summaries/notes. Fail-open per file. | `src/security/workflow/report.rs:1062` |
