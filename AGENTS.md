...
| `SecurityReviewWorkflowOptions` | Options struct for `run_security_review_workflow` orchestrator: `include_prompts`, `include_findings`, `run_filename_preflight`, `run_content_preflight`, `hunk_local_content_preflight`, `max_findings`, `max_prompts`. Defaults are conservative and bounded. | `src/security/workflow/types.rs` |
...
| `/security-review` command | TUI slash command with flags: `--changed` (shorthand for `--base HEAD`), `--base <ref>`, `--json`, `--prompts-only`, `--findings-only`, `--no-content`, `--no-filename`, `--max-findings N`, `--max-prompts N`. Command handler: `parse_security_review_args()` and `run_security_review_command()` in `src/security/workflow/report.rs`. Wired in `src/tui/app/mod.rs`. | `src/tui/app/mod.rs`, `src/security/workflow/report.rs` |
