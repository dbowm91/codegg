...
- **Capability enforcement**: `execute_python_script()` calls `check_compatibility()` BEFORE execution — denied capabilities block execution. Snapshots are taken for ALL modes (Analyze, Transform, Verify), not just Transform. Analyze and Verify modes fail if ANY workspace files change. Capability checks distinguish file reads from file writes: `has_file_read` with `read_workspace`, `has_file_write` with `write_workspace`, destructive ops with `destructive_fs`.
