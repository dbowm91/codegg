...
- **Security review workflow**: `src/security/workflow.rs` implements a security review vertical slice: parses unified diff hunks (`parse_changed_hunks`), applies path exclusions (`is_security_review_excluded_path`), selects deterministic `securityContext` presets (`select_security_preset`), builds review targets and `securityContext` request payloads (`build_security_review_targets`, `build_security_context_request`), converts risk markers to review prompts (`prompts_from_security_context`), and assembles reports with an explicit "not confirmed findings" note (`assemble_security_review_report`). Top-level API: `plan_security_review_from_diff(diff, repo_root)`. Key types: `ChangedHunk`, `SecurityReviewTarget`, `SecurityTargetReason`, `SecurityReviewPrompt`, `SecurityReviewFindingStub`, `SecurityReviewReport`. Exclusions: `vendor/`, `third_party/`, `target/`, `dist/`, `build/`, `node_modules/`, `*.min.js`. NOT excluded: `Cargo.toml`, `Cargo.lock`, `build.rs`. Risk markers become review prompts, NEVER findings. `call_depth` is always 0. Findings are always empty in this vertical slice. Async target discovery (`discover_targets_from_diff`) reuses `build_security_review_targets` and `build_file_level_security_review_target` for consistent positioned targets with `column: Some(1)`. Per-file patches without `diff --git` headers are handled by `parse_changed_hunks_for_file`. Preflight checks use filename-hint names (`secret_filename_hint_scan`, `unsafe_filename_hint_scan`) — they scan file names only, not contents. Prompt sources are distinguishable: `source: changed_hunk` for planning, `source: securityContext.risk_marker` for marker-driven prompts.
- **Evidence-based security finding synthesis**: `synthesize_evidence_based_findings()` groups evidence by file/line, applies `is_finding_eligible()` gate (requires 2+ evidence dimensions), and produces `SecurityReviewFinding` with `SecuritySeverity`/`SecurityConfidence` enums. Marker-only evidence never creates findings. Content preflight checks are local/deterministic (secrets, unsafe, process exec, SQL interpolation, weak crypto). Findings are defensive review outputs, not proof of exploitability.
