...
- **Tool Programs (M006-M020)**: `tool_program` foreground model tool submits restricted-Python programs. Read-only palette: `read`, `glob`, `grep`, `list`, `diff` (`diff` promoted by expansion M001), `repo_search` (`repo_search` promoted by expansion M002 as an explicitly nondeterministic external read: `DirectOrProgrammatic`/`ReadOnly`/`Idempotent`, output schema, no broker retry, program-call cache disabled, `ExternalUntrusted` provenance) with `DirectOrProgrammatic` caller policy, plus hidden `ProgrammaticOnly` operation-scoped adapters `git_read` (`status`/`diff`/`log`/`branches` via `GitExecutionService`) and `lsp_read` (`diagnostics`/`documentSymbol`/`workspaceSymbol`/`hover`/`goToDefinition`/`findReferences` via the canonical `LspTool`; promoted by expansion M003 as explicitly workspace/server-state-dependent reads: `ProgrammaticOnly`/`ReadOnly`/`Idempotent`, output schema, no broker retry, program-call cache disabled, `LocalTrusted` for git / `LocalUntrusted` for LSP). Programmatic admission requires the M001 eligibility matrix (explicit caller policy + read-side effect + output schema + bounded I/O + execution-context workspace policy) — `ToolCategory::ReadOnly` alone is never sufficient. The multiplexed `git`/`lsp` tools stay `DirectOnly`; only the narrow adapters are program-callable. Manifest resolution validates tool availability before execution. `ToolProgramExecutor` uses real `ToolBroker` via `BrokerAdapter`. `ProgramCallCache` caches read-only results with content/policy-aware keys. Output schemas defined for all palette tools.
...
- **Manifest resolution gates submission**: Tools without a program-capable policy (`DirectOrProgrammatic` or hidden `ProgrammaticOnly`) or output schemas are rejected before job creation.
- **ToolProgramTool is DirectOnly**: Only the agent loop can submit programs. Programs themselves can only call `DirectOrProgrammatic` tools and the hidden `ProgrammaticOnly` adapters (`git_read`, `lsp_read`).
