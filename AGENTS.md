...
| egglsp-test-server bin target | `crates/egglsp-test-server/src/main.rs` (built by `egglsp`) | Fake LSP server binary for deterministic integration testing and scenario-engine self-tests. Reads Content-Length framed JSON-RPC, executes scripted scenarios, writes machine-readable transcripts. |
...
| Root composite test count | 16 | `tests/lsp_composite_stdio.rs` — exercises `SemanticContextCollector`, `DiagnosticsCollector`, and `LspOperations` against fake server via production stack |
