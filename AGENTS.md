...
| `LspHunkSourceContextExecutor` | Real adapter implementing `HunkSourceContextExecutor` by wrapping `LspTool`. Serializes the request, injects `"operation": "hunkSourceContext"`, delegates to `LspTool::execute()`, parses the JSON response. Lives in `src/security/lsp_executor.rs` near the security workflow boundary. | `src/security/lsp_executor.rs` |
...
| `collect_hunk_source_context_all_files()` | Async. Groups hunks by file (capped at 8 files), calls `collect_hunk_source_context_for_file` per file, merges evidence/summaries/notes. Accepts an optional `HunkSourceContextExecutor`. Fail-open per file. | `src/security/workflow/report.rs:1062` |
