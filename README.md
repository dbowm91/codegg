...
- **LSP Support** - Built-in bidirectional Language Server Protocol support with server-request handling, timeout cancellation with transport-failure propagation, explicit leader/waiter initialization with shared completion fan-out, lifecycle-validated publication, quiescent shutdown with deadline-bounded (6s) cooperative cancellation and authoritative task-completion finalization (wrapper-owned oneshot completion receivers, start-registration barrier, and explicit wrapper cleanup with `ActiveTaskGuard` fallback), and code intelligence including semantic context packets and preview-only semantic checks. Phase 2 scripted stdio integration tests now live under `crates/egglsp/tests/` (11 protocol + 3 semantic + 5 service tests) plus root-crate composite tests in `tests/lsp_composite_stdio.rs` (24 tests covering semantic/security/hunk collectors, security context tool orchestration with risk filtering, call expansion, node-limit truncation, depth-limit enforcement, and diagnostic evidence, workspace-edit-preview safety, and capability-gated degradation), using the `codegg-lsp-test-server` binary target and a production harness. The Phase 3 corrective pass adds an authoritative per-process runtime owner (`LspProcessRuntime` with explicit `LspProcessIntent`), a single restart coordinator (`restart_client_coordinator` driven by persisted `LspClientDescriptor` and `RestartTrigger`), per-client generation safety with stale-event rejection, and a four-variant `LspReadinessPolicy` honored by `LspService::wait_for_readiness`; 9 deterministic scripted supervisor tests live in `crates/egglsp/tests/supervisor_restart_stdio.rs` and the real-server CI pins `rust-toolchain@1.81.0` / `basedpyright@1.13.1`.
