...
- **LSP Support** - Built-in bidirectional Language Server Protocol support with server-request handling, timeout cancellation with transport-failure propagation, explicit leader/waiter initialization with shared completion fan-out, lifecycle-validated publication, quiescent shutdown with deadline-bounded (6s) cooperative cancellation, and code intelligence including semantic context packets, preview-only semantic checks, and fail-closed capability gating. Phase 2 scripted stdio integration tests under `crates/egglsp/tests/` plus 26 root-crate composite tests in `tests/lsp_composite_stdio.rs`. **Phase 3 supervision and restart lifecycle complete for Tier 1 servers. Phase 4 complete for the exact pinned Tier 1 and Tier 2 matrix.** All five real-server jobs pass on one commit; the aggregate manifest verifies consistent run metadata, report completeness, typed operation invariants, required-operation success, shutdown traces, and exact version evidence. Compatibility outside the pinned matrix remains experimental. **Phase 4 final harness evidence and matrix closure (Passes 1–11)** — explicit per-fixture implementation expectations, request-site per-operation records, advertised-required check enforcement, type-hierarchy as three distinct operations, edit-bearing TypeScript code actions, structured shutdown traces, encoding-aware semantic-token bounds, strengthened rename smoke, full 25-operation matrix, executed-and-preserved full matrix (`target/lsp-compatibility/`), and documentation closure (`architecture/lsp.md` / `AGENTS.md` / `.opencode/skills/lsp/SKILL.md`); see `plans/lsp_phase4_final_harness_evidence_and_matrix_closure.md` for the full pass list. **Phase 5 complete:** bounded LSP context packets with provenance, budget enforcement, hunk-focused evidence, security risk tagging, agent prompt rendering, preview artifact registry, TUI summaries, and degradation policy. **Phase 5 completion hardening (Passes 1–10)** turns the prototype into a production-shaped pipeline: `LspContextPacket` is the single source of truth (legacy `SemanticContextPacket` bridges via `into_lsp_context_packet`); `ServiceLspEvidenceProvider` (`crates/egglsp/src/evidence_adapter.rs`) wires `LspEvidenceProvider` to the live `LspService` with `EvidenceOperation` provenance; `LspAgentContextInput` flows through `TurnRunInput` so the agent loop can pass workflow metadata (changed files, hunks, active file, optional `ModelTier` override) to `LspTool::lsp_context_for_agent_with_input()`; `LspTool` owns an internal `PreviewArtifactRegistry` and every `LspToolOutput` carries a `preview_id: Option<String>`; the security review integration requests context via `LspContextRequest::Review` with `LspRiskMode::Aggressive` and `SecurityEvidenceSummary.public_api_fanout` counts distinct reference files; the hunk navigation bridge (`crates/egglsp/src/hunk_context.rs::hunk_response_to_context_items`) tags converted items with `AgentContextSource::Hunk` and `LspEvidenceFreshness::PossiblyStale` on truncation; `model_tier_for_profile` is exposed in production with tier-gated section filtering (Small omits refs/hover, Workhorse adds them, Frontier broader); `LspTuiSummary` is extended additively with `total_items`, `fresh_count`, `stale_freshness_count`, `possibly_stale_count`, `preview_ids`, and `unsupported_operations` plus `LspTool::lsp_summary_detail()`; 7 production-seam tests in `tests/phase5_context_integration.rs` exercise the adapter, security bridge, hunk bridge, and registry end-to-end via `MockProvider`; final regression closure: `cargo fmt` + `cargo check --workspace --all-targets --all-features` (no new errors/warnings) + `cargo test --workspace` (1610 pass, 1 pre-existing flake unrelated to Phase 5). Total Phase 5 test count: 40 in `tests/phase5_context_integration.rs` (33 pre-existing + 7 new). See `plans/lsp_phase5_completion_hardening_and_workflow_depth.md` for the full pass list. **Phase 5 final closeout and stabilization (Passes 1–9)** narrows the surface and reasserts safety invariants at the boundary. Pass 1 stabilizes the `agent_loop_harness::test_no_follow_up_latency` flake (15s wall-clock bound + deterministic provider-call-count behavioral assertion; 10/10 serial and 5/5 parallel stability). Pass 2 propagates live preview IDs through actual `renamePreview` / `formatPreview` / `sourceActionPreview` / `codeActionPreview` outputs via a new `preview_metadata: Option<PreviewMetadata>` envelope on `LspToolOutput` carrying `not_applied` / `edit_count` / `affected_files` / `stale_base` (10 unit tests + 4 live integration tests). Pass 3 hardens `ServiceLspEvidenceProvider` provenance with a `consume_provenance_for(expected_operation)` guard and a sequential-call contract pinned on `LspEvidenceProvider` (5 new tests). Pass 4 adds canonical model drift guardrails via `crates/egglsp/src/bridges.rs` with `semantic_context_to_lsp_items` / `lsp_packet_to_security_summary` / `lsp_packet_to_tui_summary` (9 new tests). Pass 5 extracts `DefaultTurnRuntime` LSP context assembly into `resolve_lsp_context_tier` and `assemble_lsp_context_for_turn` for unit testing (7 new tests). Pass 6 adds 6 hunk/security bridge production-seam tests driving the real `LspTool` stack against the fake server. Pass 7 verifies the no-mutation / no-`workspace/executeCommand` invariant with 4 hash-based tests on real temp files. Pass 8 records the full regression evidence (1627+ tests pass, 1 pre-existing flake unrelated to Phase 5). Total Phase 5 closeout test count: 53 in `tests/phase5_context_integration.rs` (40 pre-existing + 13 new) plus 6 new in `tests/lsp_composite_stdio.rs`. See `plans/lsp_phase5_final_closeout_and_stabilization.md` for the full pass list. a manual restart API (`LspService::manual_restart_client`) that terminates the old runtime before replacement; shared crash-cycle restart budget with lazy healthy-interval reset (`increment_restart_attempts` / `reset_restart_attempts_if_healthy_inherent`); retained stale diagnostics across restart (`snapshot_diagnostics_for_restart` / `install_retained_diagnostics`) with `post_restart = generation > 1`; observed-cycle progress readiness (`ProgressState.completed_cycle` required by `wait_for_progress_end`); validated restart configuration (`LspRestartPolicyConfig::try_to_domain`); real-server stderr capture via runtime-backed harness (`RealServerHarness` owns `Arc<LspClient>` + `LspProcessRuntime`, uses `spawn_process_runtime` from `egglsp::runtime`, and exposes `shutdown_and_collect()` returning `HarnessShutdownResult::{Graceful, ForceKilled, TimeoutExpired}` with real `stderr_tail` from `LspProcessRuntime::stderr_tail_capped(20)`); production readiness primitives in smoke tests (`client.wait_for_first_diagnostics()` for `WaitForDiagnosticsOrTimeout`, `client.wait_for_progress_end()` for `WaitForProgressEndOrTimeout`); 4 named harness tests (`smoke_harness_captures_stderr`, `smoke_harness_force_kills_hung_server`, `progress_readiness_failure_is_reported`, `empty_diagnostics_readiness_passes`); and a supervised-only constructor invariant (`LspService::new_arc` is the only public production constructor). 13 deterministic scripted supervisor/restart tests live in `crates/egglsp/tests/supervisor_restart_stdio.rs` (covering the 10-pass restart ownership, manual supersession, runtime-install rejection, post-spawn cleanup, single generation owner, manual back-to-back deadlock-free restarts, degraded-as-live-outcome, and empty `publishDiagnostics` readiness). Pass 1 (restart ownership slot integrity) keeps the per-key slot installed during cancellation so `RestartCompletion::Finished` is the ownership release boundary; the final closure inverts the handshake — the slot is **removed** from the per-key map BEFORE `Finished` is broadcast on the watch channel (remove-before-signal ordering), so `Finished` is observable only after the slot is provably free and waiters no longer need a separate `verify_slot_free` post-check. Pass 2 (manual pre-wait generation capture) snapshots the current generation BEFORE cancelling the in-flight owner so a generation advance during the wait is observable. Pass 3 (publication boundary cancellation policy) treats post-publication lease-token cancellation as non-aborting — the coordinator continues to a coherent `Ready` or `Degraded` outcome. 13 deterministic scripted supervisor/restart tests live in `crates/egglsp/tests/supervisor_restart_stdio.rs` and 6 restart-coordinator unit tests plus 4 Pass-11 adversarial release-ordering unit tests (`finished_is_not_observable_until_slot_is_removed`, `drop_fallback_removes_before_finished`, `old_owner_release_does_not_signal_for_new_owner`, `completion_channel_close_without_finished_is_error`) plus 1 Pass-12 abort-while-blocked cancellation-safety unit test (`cancelled_async_release_falls_back_to_drop_cleanup`) and 2 Pass-12 owner-id diagnostic unit tests (`completion_channel_close_error_names_owner`, `completion_timeout_error_names_owner`) live in `crates/egglsp/src/restart.rs`. The real-server CI pins `rust-toolchain@1.81.0` / `basedpyright@1.13.1`. **Pass 12 (Phase 3 final cleanup) — async release cancellation safety**: `RestartLease::release_async` commits `released = true` **inside** the ownership-map lock block, so a future cancelled or task aborted while parked on the lock await leaves `released == false` and routes cleanup to `Drop`'s safety fallback (which already runs the remove-before-signal ordering). Both `RestartOwnerWaiter::wait` error variants now embed the in-flight `owner_id` for caller diagnostics. **Phase 4 broader server compatibility and higher-level capability adoption is now complete**: a Tier 2 server matrix of `gopls` (v0.16.1), `typescript-language-server` (v4.3.3), and `clangd` (v18.1.3) joins the existing Tier 1 profiles (`rust-analyzer`, `basedpyright`) and passes end-to-end smoke tests under the `lsp-real-server-tests` feature; the normalized capability model in `crates/egglsp/src/capability.rs` now spans declaration, implementation, document highlights, signature help, workspace symbols, completion, semantic tokens, rename, code actions, and formatting (with separate push/pull diagnostics, an `LspCapabilityDetails` DTO for option-level data such as code-action kinds and trigger characters, and a `SemanticTokenLegendSnapshot`); read-only navigation operations (`declaration`, `implementation`, `documentHighlights`, `signatureHelp`, `workspaceSymbols`, `completion`, `semanticTokens`) are exposed as typed `LspOperations`/`LspClient` APIs and capability-gated `lsp_tool` operations that return bounded DTOs (`SignatureHelpSummary`, `CompletionCandidate`, `DecodedSemanticToken`); mutation-producing operations (`renamePreview`, `codeActions`/`codeActionPreview`, `formatPreview`) stay strictly preview-only via `RenamePreview`, `CodeActionSummary`, `CodeActionPreview`, and `FormattingPreview` DTOs that apply edits to an in-memory snapshot and never write to disk; `workspace/executeCommand` is **never** invoked (the server-request dispatcher rejects `workspace/applyEdit` with `applied: false` and no codepath calls `executeCommand`); the CI matrix at `.github/workflows/lsp-real-server.yml` runs one server per job with pinned versions (`gopls@v0.16.1` on Go 1.22.5, `typescript-language-server@4.3.3` + `typescript@5.5.4` on Node 20, `clangd@18` from the LLVM apt repo), uploads `target/lsp-compatibility/` artifacts per job, and is opt-in via `workflow_dispatch`, weekly schedule, or pushes that touch `crates/egglsp/**`, `src/lsp/**`, or the workflow itself; and higher-level evidence (signature help, completion, semantic tokens, rename/code-action/formatting previews) is surfaced selectively through `lsp_tool` operations with per-operation caps and health/generation/freshness metadata preserved on every result. **Phase 4 final reporting + semantic validation closure** adds: typed checked/raw operation split (`prepare_rename_unchecked` / `rename_preview_unchecked` / `format_preview_unchecked` / `code_actions_unchecked`); rename-when-prepareRename-unsupported path that inspects `CapabilityDecision` and falls back to `textDocument/rename` directly; real `decode_semantic_tokens` exercise in the smoke harness (asserts per-token `(line, col, length)` validity and that the decoded token types match the server's `legend.tokenTypes`); per-server actionable shutdown evidence — `ForceKilled` / `TimeoutExpired` outcomes are classified against the fixture's `shutdown_requirement` (so clangd's daemon-mode hang becomes a `KnownLimitation` with a stderr-tail excerpt) and `assert_required_checks` no longer fails on `Skipped`/`Unsupported` first-class statuses; `LspCompatibilityReport` carries an `operation_support: Vec<LspOperationCompatibility>` field with `advertised` / `exercised` / `request_succeeded` / `semantic_assertion_passed` flags per operation; the TypeScript fixture now contains a real `Greeter` interface and a `Person` class implementing it (driving `textDocument/implementation` end-to-end), the Rust fixture adds a `Greeter` trait and a `Person` struct (driving `textDocument/prepareTypeHierarchy` + subtype matching), and the clangd fixture drives `textDocument/implementation` from `include/widget.hpp`. **Phase 4 final evidence-integrity cleanup (Passes 1–10)** closes the remaining evidence-integrity gaps: `OperationOutcome` at every request site replaces string parsing; `RenameExpectation` is now typed (rename checks fail on null/zero-edit responses); `LspShutdownTrace` carries 9 granular protocol/runtime fields plus a `ProtocolShutdownTrace`; `assert_required_checks` walks typed `LspOperationCompatibility` records directly (no check-name parsing); coarse `LspSemanticOperation::TypeHierarchy` is gone — coverage is exclusively from `prepare`/`supertypes`/`subtypes`; TypeScript code-action fixture proves an edit-bearing action; `LspClient::position_encoding()` exposes the live negotiated encoding (`Utf8`/`Utf16`/`Utf32`) and semantic-token bounds use it; `RealServerFixture::requirement_for(op)` derives fixture-aware fallback requirements; `update_matrix_manifest()` writes `target/lsp-compatibility/matrix-manifest.json` per server with commit SHA + `GITHUB_RUN_ID` + per-server artifact paths, and the CI workflow adds a `matrix-summary` job that aggregates the manifests. All 11 closure invariants satisfied; Phase 2 + Phase 3 regression suites remain green. See `plans/lsp_phase4_final_evidence_integrity_cleanup.md` for the full pass list.
