...
- **Plugin System** - WASM-based plugin extensibility with process-backed command plugins (Phase 4/6): project-local slash commands can declare `runtime: process` to run executables with structured JSON output, timeout, and output capping. Runtime abstraction layer (`src/plugin/runtime/`) provides a `PluginRuntime` trait for process and future WASM execution, with unified `PluginResponse` type and hardened registry semantics
