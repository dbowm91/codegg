...
An optional `hunkSourceContext` evidence pass (`enable_hunk_source_context` in workflow options) collects hunk navigation evidence (enclosing symbols, diagnostics, definitions, references) for changed files and injects it into the evidence-based synthesis. A `HunkSourceContextPolicy` decides whether to invoke `hunkSourceContext` based on file extension, patch size, and hunk count. Errors are fail-open: noted but never blocking. `SecurityEvidenceKind::HunkNavigation` evidence is recognized by `is_finding_eligible()` as a supporting dimension. Integration is deterministic (not model-initiated) and single-file only; multi-file patches are split per-file. Semantic collection is first-hunk-centered.
