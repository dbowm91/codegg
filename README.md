...
- **Command Intent and Planning** - Agent-authored commands and scripts pass through a classification/planning/routing pipeline: `classify_command()` identifies intent families (test, git, search, python, build, raw shell), `plan_execution()` maps intents to execution backends and generates permission requests, and `resolve_routing()` routes to concrete subsystems (test runner, egggit, python scripting, managed process, raw shell). Python scripting is first-class with Analyze/Transform/Verify modes, static risk analysis, and async execution with timeout/cancellation support.
