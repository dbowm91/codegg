...
- **Human Shell** - Run shell commands with `!command` (ephemeral, hidden from model) or `!!command` (auto-promoted into context). Bounded output storage, safety policy blocking destructive commands, and TUI commands for listing, viewing details, including, rerunning, and killing commands. Shell detail view shows full metadata with promoted state and truncation info. Phase 1 of the shell-output projection pipeline adds a parallel `CommandOutputStore` for durable raw stdout/stderr retention with stable `cmd://<id>/<stream>` handles and a single `default_command_projection` seam. See `architecture/human_shell.md` and `plans/shell_output_projection_rtk_roadmap.md`.
