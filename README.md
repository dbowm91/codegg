...
- **Human Shell** - Run shell commands with `!command` (ephemeral, hidden from model) or `!!command` (auto-promoted into context). Bounded output storage, safety policy blocking destructive commands, and TUI commands for listing, viewing details, including, rerunning, and killing commands. Shell detail view shows full metadata with promoted state and truncation info. The shell-output projection pipeline adds a parallel `CommandOutputStore` for durable raw stdout/stderr retention with stable `cmd://<id>/<stream>` handles (Phase 1), the `CommandOutputProjector` trait with `RawProjector` / `TruncatedProjector` / `ErrorRetentionProjector` plus a centralised `ProjectionSelector` and redaction-hook placeholder (Phase 2). Native structured projectors for Git and Rust toolchains (Phase 3). RTK discovery, eligibility classification, and real `RtkProjector` invocation (PostProcess/Wrapper modes) with strict wrapper grammar fallback and structured `ProjectionRawSemantics` (Phase 5+6). Projection contract (Phase 09): `ProjectionResult` carries `projection_id`, `source_spans`, `redaction_records`, and `rtk_metadata`; `ProjectionRecord` in run_store persists full projection metadata with promotion decisions; `evaluate_promotion()` for budget/redaction/span-aware promotion; `PythonProjector` for first-class Python projection. See `architecture/human_shell.md` and `plans/shell_output_projection_rtk_roadmap.md`.
- **Command Intent and Planning** - Agent-authored commands and scripts pass through a classification/planning/routing pipeline: `classify_command()` identifies intent families (test, git, search, python, build, raw shell), `plan_execution()` maps intents to execution backends and generates permission requests, and `resolve_routing()` routes to concrete subsystems (test runner, egggit, python scripting, managed process, raw shell). Active routing dispatches to canonical subsystems (TestRunner, PythonScript) rather than raw shell. Ownership is truthful: `RunOwnership::DelegatedBackend` requires a real `RunId` proof from the delegated subsystem; raw-shell executions are recorded as `RunKind::RawShell` regardless of intent, with semantic intent preserved separately in routing metadata.
