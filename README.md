...
An optional `hunkSourceContext` evidence pass (`--hunk-context` flag, or `enable_hunk_source_context` in workflow options) deterministically executes `hunkSourceContext` for changed files via the `HunkSourceContextExecutor` trait and injects hunk navigation evidence (enclosing symbols, diagnostics, definitions, references) into the evidence-based synthesis. `LspHunkSourceContextExecutor` (`src/security/lsp_executor.rs`) is the real adapter wrapping `LspTool`. A `HunkSourceContextPolicy` decides whether to invoke `hunkSourceContext` based on file extension, patch size, and hunk count. Policy skip decisions are routing metadata, never security evidence — only real `HunkSourceNavigationResponse` produces `HunkNavigation` evidence. Errors are fail-open: noted but never blocking. `SecurityEvidenceKind::HunkNavigation` evidence is recognized by `is_finding_eligible()` as a supporting dimension but requires `RiskMarker` or `Preflight` alongside to form a finding. Integration is single-file only; multi-file patches are split per-file. Semantic collection is first-hunk-centered.
