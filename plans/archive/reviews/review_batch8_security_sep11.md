> **Archived 2026-09-13.** Superseded by `plans/review_batch8_security.md` (2026-09-13, current batch structure; `identity.md` coverage moved to batch 5, `lsp_disk_cache_threat_model.md` to batch 9). Verified during reconciliation that its line-number findings predate the architecture-doc rewrite (e.g. it cites `permission.md:1574`, but the current doc is 414 lines) and are obsolete. Retained for traceability only.
