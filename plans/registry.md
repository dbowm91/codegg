...
| Tool-selection advisor sequence-encoder experiment | M002 current-state advisor context v2 | active | `plans/implementation/tool-selection-advisor-sequence-encoder-experiment/003-current-state-advisor-context-v2.md` | Replace stale original-prompt-only projection with bounded current Goal/Task/error context. |
...
**Tool-selection advisor sequence-encoder experiment gate:** `plans/subsystems/tool-selection-advisor-sequence-encoder-experiment-roadmap.md` is active. P001 evidence/preregistration polish is closed; M001 selected Candle but is conditionally closed pending real local pretrained reference-asset evidence; M002 current-state advisor context v2 is ready. M003 true sequence-encoder ranking is blocked on that M001 condition plus M002; M004 hybrid retrieval is blocked on M003; M005 clean offline qualification is blocked on P001+M002+M003+M004. The existing live-primary-model M004 remains blocked unless M005 records a positive disposition. The experiment must not mutate frozen C001 test/family-holdout labels or weaken ADR-0009 locality/authority constraints.
...
| Tool-selection advisor sequence-encoder experiment | M001 Rust sequence-encoder framework/asset spike | conditionally closed | `plans/closure/tool-selection-advisor-sequence-encoder-experiment/002-status.md` | `31d16ad`; Candle fixture qualified, but real TinyBERT/MiniLM local asset evidence is required before M003. M002 remains ready. |
