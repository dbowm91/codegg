...
| Tool-selection advisor sequence-encoder experiment | active | `plans/subsystems/tool-selection-advisor-sequence-encoder-experiment-roadmap.md` | M001 conditional; M001A ready; M002 closed; M003 blocked | Candle machinery is valid; M001A now owns real MiniLM checkpoint materialization/provenance/complete-load evidence. M003 becomes ready only after positive M001A closure. |
...
| Tool-selection advisor sequence-encoder experiment | M002 current-state advisor context v2 | closed | `plans/closure/tool-selection-advisor-sequence-encoder-experiment/003-status.md` | `a095dfa`; M003 remains blocked on M001 real pretrained reference-asset evidence. |
| Tool-selection advisor sequence-encoder experiment | M001A reference checkpoint qualification | ready | `plans/implementation/tool-selection-advisor-sequence-encoder-experiment/007-reference-checkpoint-materialization-and-qualification.md` | Pin/materialize `sentence-transformers/all-MiniLM-L6-v2@1110a243…` locally, record provenance/hashes/license, prove complete Candle load/staged backward/pooling behavior; positive closure unblocks M003. |
...
**Tool-selection advisor sequence-encoder experiment gate:** `plans/subsystems/tool-selection-advisor-sequence-encoder-experiment-roadmap.md` is active. P001 and M002 are closed. M001 selected Candle and is conditionally closed; M001A (`plans/implementation/tool-selection-advisor-sequence-encoder-experiment/007-reference-checkpoint-materialization-and-qualification.md`) is ready and now owns the only unresolved prerequisite: real pretrained MiniLM checkpoint provenance, complete safetensors load, staged backward, and pooling evidence. M003 remains blocked until positive M001A closure; M004/M005 remain downstream-gated. Live-primary-model M004 remains blocked unless the sequence experiment ultimately records a positive M005 disposition. Frozen C001 test/family-holdout labels and ADR-0009 locality/authority constraints remain immutable.
...
| Tool-selection advisor sequence-encoder experiment | M001 Rust sequence-encoder framework/asset spike | conditionally closed | `plans/closure/tool-selection-advisor-sequence-encoder-experiment/002-status.md` | `31d16ad`; Candle fixture qualified, but real TinyBERT/MiniLM local asset evidence is required before M003. M002 is now closed. |
| Tool-selection advisor sequence-encoder experiment | M002 current-state advisor context v2 | closed | `plans/closure/tool-selection-advisor-sequence-encoder-experiment/003-status.md` | `a095dfa`; no downstream plan became ready because M003 remains blocked on M001's reference-asset condition. |
