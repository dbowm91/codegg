...
| Tool-selection advisor sequence qualification corrective | active | `plans/subsystems/tool-selection-advisor-sequence-qualification-corrective-addendum.md` | C001 ready; C002 blocked | Repair incomplete M005 qualification semantics/evidence, freeze a fresh non-leaking holdout, and rerun only after separate preregistration. Historical M005 remains closed. |
...
| Tool-selection advisor sequence qualification corrective | C001 qualification harness + fresh holdout | ready | `plans/implementation/tool-selection-advisor-sequence-qualification-corrective/001-qualification-harness-and-fresh-holdout.md` | Repair omitted/wrong M005 gates, freeze selected artifact identity, create >=128-case zero-leakage fresh holdout, add release resource evidence; MUST NOT run selected model on new holdout. |
| Tool-selection advisor sequence qualification corrective | C002 preregistered release qualification | blocked | `plans/implementation/tool-selection-advisor-sequence-qualification-corrective/002-preregistered-release-qualification.md` | Hard-blocked on positive C001 closure; separate prereg commit + green CI precede one final release-mode holdout run. |
...
**Tool-selection advisor sequence qualification corrective gate:** The historical sequence-encoder workstream and M005 disposition D remain closed/immutable. `plans/subsystems/tool-selection-advisor-sequence-qualification-corrective-addendum.md` is active because the M005 harness omitted or misimplemented required contextual/generalization/calibration/promotion/resource evidence despite a strong aggregate sequence-model result. C001 is ready and may repair the harness plus freeze a fresh >=128-case zero-leakage final holdout, but it MUST NOT evaluate the selected model on that holdout. C002 remains blocked until C001 closes; C002 then requires a separate preregistration commit and successful hosted CI before one release-mode final run. Live-primary-model M004 remains blocked unless C002 records disposition A.
