...
| Tool-selection advisor sequence-encoder experiment | active | `plans/subsystems/tool-selection-advisor-sequence-encoder-experiment-roadmap.md` | M001-M003 closed; M004 ready | M003 selected packed-marker head-only ranker (dev MRR 0.823 vs BM25 0.515); top-layer local resource stop recorded. M004 retrieval/resource gates remain. |
...
| Tool-selection advisor sequence-encoder experiment | M005 clean offline sequence-encoder qualification | ready | `plans/implementation/tool-selection-advisor-sequence-encoder-experiment/006-clean-offline-sequence-encoder-qualification.md` | P001, M002, M003, and M004 closed; preregister the final comparison and retain a negative disposition if expanded retrieval/quality/resource gates fail. |
...
**Tool-selection advisor sequence-encoder experiment gate:** `plans/subsystems/tool-selection-advisor-sequence-encoder-experiment-roadmap.md` is active. P001, M001, and M002 are closed. M001A (`plans/closure/tool-selection-advisor-sequence-encoder-experiment/007-status.md`; implementation `ab3e9dfd`) is conditionally closed with the MiniLM asset, complete load, deterministic forward, scoped head-only training, and pooling evidence qualified. M001B (`plans/closure/tool-selection-advisor-sequence-encoder-experiment/008-status.md`; implementation `1633bfce`) is closed positively via the CodeGG-owned composite-norm encoder (101/101 weights, parity max 2.3e-06, correctly-scoped top-layer updates). M003 is ready for all three training stages through the M001B path; M004/M005 remain downstream-gated on M003. Live-primary-model M004 remains blocked unless the sequence experiment ultimately records a positive M005 disposition. Frozen C001 test/family-holdout labels and ADR-0009 locality/authority constraints remain immutable.
