...
| Tool-selection advisor post-closure corrective | M001 dataset/split/consent integrity | closing | `plans/implementation/tool-selection-advisor-post-closure-corrective/001-dataset-split-and-consent-integrity.md` | Production implementation landed in `66bf63e`; closure evidence is being recorded. |
...
**Tool-selection advisor corrective gate:** Accepted `plans/adrs/ADR-0009-local-tool-advisor-boundaries.md` remains controlling and the original `plans/subsystems/tool-selection-advisor-roadmap.md` M001-M005 closure records remain immutable historical evidence. Post-closure review found that `hashed-linear-v1` is only a lexical baseline rather than the planned contextual encoder, `promote` is currently wired reactively inside `tool_search` rather than before provider-definition finalization, qualification remains smoke-scale/no-live-primary-model, the `tool-advisor` Cargo feature does not yet isolate a neural runtime, and training-event consent has two representations. `plans/subsystems/tool-selection-advisor-post-closure-corrective-addendum.md` is in M001 closure review. Corrective M001 dataset/split/consent integrity is closing and M003 pre-turn proactive disclosure remains ready and independent; M002 contextual encoder/runtime/training is blocked on M001; M004 live small-model trajectory qualification is blocked on M001+M002+M003. Advisor use, model use, training, capture, and remote telemetry all remain opt-in/default-off.
...
| Tool-selection advisor post-closure corrective | M002 contextual encoder runtime/training | M001 must close with frozen expanded dataset, counterfactual cases, and tool-family holdouts before architecture tuning. |
