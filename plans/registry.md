...
| Tool-selection advisor post-closure corrective | active | `plans/subsystems/tool-selection-advisor-post-closure-corrective-addendum.md` | M001-M003 closed; M004 gated | M001 dataset/consent, M002 contextual encoder, and M003 pre-turn disclosure are closed; M004 requires live primary-model qualification evidence. Original M001-M005 closures remain historical. |
| Tool-selection advisor evidence-integrity corrective | active | `plans/subsystems/tool-selection-advisor-evidence-integrity-corrective-addendum.md` | C001 + C002 + C003 closed; C004 ready | Repairs cross-split content leakage, contextual optimizer/calibration correctness, and deferred-candidate truncation before any live M004 provider study. |
...
| Tool-selection advisor evidence-integrity corrective | C004 clean offline requalification | ready | `plans/implementation/tool-selection-advisor-evidence-integrity-corrective/004-clean-offline-requalification.md` | Unblocked by C001+C002+C003 closures; must produce a positive offline disposition before live provider qualification is allowed. |
...
**Tool-selection advisor corrective gate:** Accepted `plans/adrs/ADR-0009-local-tool-advisor-boundaries.md` remains controlling and all original/predecessor closure records remain immutable historical evidence. The first post-closure corrective successfully landed consent authority, optional local contextual machinery, and the correct pre-turn visibility seam, but a later audit at `71460c0` found cross-split duplicate-content leakage, incorrect contextual gradient/update normalization, missing runtime calibration, and candidate truncation before deferred eligibility. `plans/subsystems/tool-selection-advisor-evidence-integrity-corrective-addendum.md` now controls the evidence repair: C001 corpus/split integrity, C002 training math/calibration, and C003 deferred-first shortlisting are closed with frozen clean partitions (train `c67df3ca…`/dev `b804b7d8…`/test `1765ad09…`), corrected calibrated artifacts, and deferred-first shortlisting; C004 clean offline requalification is ready. The existing live-primary-model M004 plan MUST remain blocked until C004 records a positive offline disposition, after which its original operator/provider/resource prerequisites still apply. Advisor use, model use, training, capture, and remote telemetry remain opt-in/default-off.
...
| Tool-selection advisor post-closure corrective | M004 live small-model trajectory qualification | Blocked first on a positive C004 evidence-integrity closure; after that, still requires its original operator-configured live primary-model/provider calls, held-out trajectory suite, and resource measurements. |
