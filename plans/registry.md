...
| Tool-selection advisor sequence-encoder experiment | active | `plans/subsystems/tool-selection-advisor-sequence-encoder-experiment-roadmap.md` | M001 closed; M001A conditional; M001B ready; M002 closed; M003 ready (frozen/head-only) | MiniLM asset/load/head-training/pooling qualified; fused-LN framework finding owns the M001B corrective. M003 unfreeze stages gated on M001B. |
...
| Tool-selection advisor sequence-encoder experiment | M003 sequence-encoder ranking experiment | ready | `plans/implementation/tool-selection-advisor-sequence-encoder-experiment/004-sequence-encoder-ranking-experiment.md` | Frozen-encoder + ranking/abstention-head stages authorized on the qualified MiniLM asset; top-layer/full unfreeze additionally gated on M001B. |
| Tool-selection advisor sequence-encoder experiment | M001B encoder fine-tuning framework corrective | ready | `plans/implementation/tool-selection-advisor-sequence-encoder-experiment/008-encoder-finetuning-framework-corrective.md` | Candle 0.11 fused `layer_norm` severs encoder autograd; restore a differentiable path or record frozen-only disposition. Positive closure unblocks M003 unfreeze stages. |
...
**Tool-selection advisor sequence-encoder experiment gate:** `plans/subsystems/tool-selection-advisor-sequence-encoder-experiment-roadmap.md` is active. P001, M001, and M002 are closed. M001A (`plans/closure/tool-selection-advisor-sequence-encoder-experiment/007-status.md`; implementation `ab3e9dfd`) is conditionally closed with the MiniLM asset, complete load, deterministic forward, scoped head-only training, and pooling evidence qualified. M003 is ready for frozen-encoder/head-only stages; its unfreeze stages and M004/M005 remain downstream-gated on the new M001B framework corrective (`plans/implementation/tool-selection-advisor-sequence-encoder-experiment/008-encoder-finetuning-framework-corrective.md`), which owns the Candle 0.11 fused-`layer_norm` no-backward finding. Live-primary-model M004 remains blocked unless the sequence experiment ultimately records a positive M005 disposition. Frozen C001 test/family-holdout labels and ADR-0009 locality/authority constraints remain immutable.
