...
| Development verification and release | active | `plans/subsystems/development-verification-release-final-evidence-closure-addendum.md` | M008 active; M009 closed; M007 remains closed | M008 owns the pre-existing `items_after_test_module` correction; M009's stale nine-agent test expectations are closed. |
...
5. Development verification M008 and M009 are separate hosted-verification corrective milestones; M008 remains active, while M009 closed only stale built-in-agent test expectations without altering agent assets or runtime behavior.
