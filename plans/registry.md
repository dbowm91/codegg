...
| Identity, authorization, and audit | active | `plans/subsystems/identity-authorization-audit-roadmap.md` | M004 closed, M005 ready | M005 chained behind M004 closure, now satisfied. |
...
| Identity, authorization, and audit | M005 — audit instrumentation and attribution closure | `plans/implementation/identity-authorization-audit/005-audit-instrumentation-attribution-closure.md` | M004 audit foundation closed; durable runs, jobs, worktrees, assets, and provider connections already expose correlation identities. |
...
These six plans may be implemented in parallel if agents avoid overlapping documentation/module edits. A plan moves to `active` only when implementation actually begins.
...
2. Identity/auth/audit is intentionally sequential: M001 domain -> M002 transport identity -> M003 daemon authorization/attribution (closed) -> M004 audit store (closed) -> M005 instrumentation closure (ready).
...
4. Project collaboration M001 cannot begin before identity/audit M005 and presence/observation M003 close. Chat UI and structured actions follow M001. Free text is never an execution interface.
5. Interactive-process M001 may proceed independently. M002 protocol follows the PTY engine; M003 TUI/legacy disposition follows M002.
...
| Project collaboration | M001 channel/message protocol | Identity/audit M005 + presence-observation M003 closures. |
...
| Interactive process sessions | M002 attach/resume protocol | PTY M001 closure. |
