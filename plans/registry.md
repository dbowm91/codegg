...
| Agent context and discovery surface | active | `plans/subsystems/agent-context-discovery-surface-roadmap.md` | M001 + M002 ready | Existing MCP/MemoryStore/tool-surface owners are in place; M001 and M002 may execute in parallel. |
...
|---|---|---|---|---|
| Agent context and discovery surface | M001 MCP resource/prompt projection | ready | `plans/implementation/agent-context-discovery-surface/001-mcp-resource-and-prompt-projection.md` | Existing McpService, runtime-assets MCP contribution, and closed progressive tool-surface work satisfy dependencies. |
...
**Agent extension/interoperability execution gate:** `plans/subsystems/agent-context-discovery-surface-roadmap.md` has M001 MCP resource/prompt projection and M002 bounded curated-memory retrieval ready and independent. `plans/subsystems/plugin-ecosystem-interoperability-roadmap.md` has M001 first-class plugin tools and M002 Agent Plugins 1.0 import ready and independent. Playwright M003 and catalog/install M004 are intentionally blocked on M002 so they consume the final portable-package/install contract rather than creating temporary package paths. None of this work reopens the closed context-continuity transcript/vector-search scope or the closed coding-agent tool-surface corrective milestones.
