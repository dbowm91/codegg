...
**Tool-selection advisor execution gate:** Accepted `plans/adrs/ADR-0009-local-tool-advisor-boundaries.md` establishes an optional local advisory-only model: disabled/no-model operation remains canonical, inference/training stay pure Rust/local-first, advisor output cannot widen authority, and remote training telemetry is separately explicit opt-in with no default endpoint. `plans/subsystems/tool-selection-advisor-roadmap.md` is closed. M001-M005 are closed with `plans/closure/tool-selection-advisor/001-status.md` through `005-status.md`; rerank/promote remain explicitly experimental and default-off because downstream primary-model trajectory evidence is not claimed. No later registered advisor plan remains blocked or ready.
