...
pub fn detect_overflow(messages: &[Message], context_limit: usize, reserved: usize) -> bool {
    context_tokens(messages, None) > context_limit.saturating_sub(reserved)
}
...
/// The effective capacity used by context preparation.  Keeping this
/// calculation beside compaction prevents callers from independently
/// subtracting output/tool reservations before invoking the policy engine.
...
impl ContextCapacity {
    pub fn new(model_limit: usize, reserved_output_tokens: usize) -> Self {
        Self {
            model_limit,
            reserved_output_tokens,
            available_context_tokens: model_limit.saturating_sub(reserved_output_tokens),
        }
    }
}

/// Typed result states for the canonical context/compaction boundary.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CompactionStatus {
    ...
    ProviderFailure,
    InvalidHistoryOrBudget,
    Cancelled,
}

/// Request consumed by the canonical context owner.  The agent loop supplies
/// turn sequencing and the already-resolved provider/session context; policy
/// decisions and message construction remain in this module.
pub struct ContextCompactionRequest<'a> {
    ...
    pub auto: bool,
    pub prune: bool,
    pub compaction_config: Option<&'a crate::config::schema::CompactionConfig>,
...
/// Return the model-aware token estimate used by the compaction boundary.
pub fn context_tokens(messages: &[Message], model: Option<&str>) -> usize {
    ContextTracker::new(usize::MAX, 0.0)
        .with_model(model.map(ToOwned::to_owned))
        .estimate_tokens_for_messages(messages)
}

/// Decide whether the canonical context owner needs to do work for a request.
pub fn needs_context_compaction(
    ...
    let tokens = context_tokens(messages, model);
    tokens > context_limit.saturating_sub(reserved_output_tokens)
        || (tokens as f64 > context_limit as f64 * threshold)
}
...
/// Prepare a provider-facing context through the one canonical compaction
/// policy.  This is intentionally an owned result: callers cannot partially
/// mutate AgentLoop state while policy is running.
pub async fn compact_context(request: ContextCompactionRequest<'_>) -> ContextCompactionResult {
    let tracker = ContextTracker::new(request.context_limit, request.threshold)
        .with_model(request.active_model.map(ToOwned::to_owned));
    let tokens_before = tracker.estimate_tokens_for_messages(request.messages);
    let capacity = ContextCapacity::new(request.context_limit, request.reserved_output_tokens);
    ...
    if request.context_limit == 0 || request.reserved_output_tokens >= request.context_limit {
        return ContextCompactionResult::unchanged(
            CompactionStatus::InsufficientCapacity,
            request.messages,
            capacity,
            tokens_before,
            vec![CompactionDiagnostic {
                level: CompactionDiagnosticLevel::Error,
                message: "context capacity is exhausted by the reserved output budget".into(),
            }],
        );
    }
    ...
    let mut working_tracker = ContextTracker::new(request.context_limit, request.threshold)
        .with_model(request.active_model.map(ToOwned::to_owned));
    working_tracker.add_messages(&messages);

    if working_tracker.needs_overflow_protection(request.reserved_output_tokens) && request.prune {
        messages = prune_tool_outputs(&messages, request.max_tool_result_tokens);
        changed = true;
        working_tracker.reset();
        working_tracker.add_messages(&messages);
        diagnostics.push(CompactionDiagnostic {
            level: CompactionDiagnosticLevel::Info,
            message: "tool outputs pruned before context compaction".into(),
        });
    }

    if !working_tracker.needs_compaction() {
        let status = if !changed {
            CompactionStatus::Ready
        } else {
            CompactionStatus::Compacted
        };
        let tokens_after = working_tracker.current_tokens();
    ...
    if !request.auto {
        ...
        let mut config = request
            ...
            .unwrap_or_default();
        config.context_limit = request.context_limit;
        config.threshold = request.threshold;
        config.reserved_tokens = request.reserved_output_tokens;
    ...
    let mut after_tracker = ContextTracker::new(request.context_limit, request.threshold)
        .with_model(request.active_model.map(ToOwned::to_owned));
    after_tracker.add_messages(&messages);
    let invalid_history = validate_message_invariants(&messages).is_err();
    if invalid_history {
        diagnostics.push(CompactionDiagnostic {
            level: CompactionDiagnosticLevel::Error,
            message: "compaction result failed tool-call history validation".into(),
        });
    }
    let status = if invalid_history {
        CompactionStatus::InvalidHistoryOrBudget
    } else if provider_failure.is_some() {
        CompactionStatus::ProviderFailure
    } else if after_tracker.current_tokens() > capacity.available_context_tokens {
...
/// The historical `compact_with_policy` API remains available for callers
/// that do not need cancellation.
pub async fn compact_with_policy_cancellable(
    input: CompactionInput<'_>,
    provider: Option<&dyn crate::provider::Provider>,
    context: ProviderRequestContext,
    ...
    let result = match cancellation {
        Some(token) => {
            tokio::select! {
                _ = token.cancelled() => {
                    return Err(crate::error::AppError::Provider("compaction cancelled".into()));
                }
                result = compact_with_policy(input, provider, context) => result,
            }
        }
        None => compact_with_policy(input, provider, context).await,
    };
    if cancellation_requested(cancellation) {
        return Err(crate::error::AppError::Provider(
            "compaction cancelled".into(),
...
mod tests {
    ...
    #[test]
    fn context_capacity_subtracts_reserved_output_once() {
        let capacity = ContextCapacity::new(128_000, 16_000);
        assert_eq!(capacity.available_context_tokens, 112_000);
    }

    #[tokio::test(flavor = "current_thread")]
    async fn canonical_context_result_reports_compaction() {
        let messages = (0..20)
        ...
        assert_eq!(result.capacity.available_context_tokens, 40);
    }

    #[tokio::test(flavor = "current_thread")]
    async fn canonical_context_result_reports_cancellation_without_mutation() {
        let cancellation = CancellationToken::new();
        cancellation.cancel();
        let messages = vec![Message::User {
            content: vec![ContentPart::Text {
                text: "preserve this".to_string().into(),
            }],
        }];
        let result = compact_context(ContextCompactionRequest {
            messages: &messages,
