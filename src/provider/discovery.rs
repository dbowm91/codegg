...
impl ModelDiscoveryService {
    ...
    async fn load_from_cache_or_embedded(&self) {
            ...
            if let Ok(models) = self.load_from_db(pool).await {
                if !models.is_empty() {
                    ...
                    *m = models.clone();
                    ...
                    // Set last_refresh to the latest fetched_at
        ...
        *models = crate::provider::models::embedded_models()
            .into_iter()
    ...
    pub async fn refresh(&self, registry: &ProviderRegistry) -> Vec<ModelInfo> {
        ...
        if all_models.is_empty() {
            all_models = crate::provider::models::embedded_models();
        }
...
struct ModelInfoInternal {
    ...
    pub variants: Vec<crate::provider::ModelVariant>,
    pub fetched_at: i64,
}
...
mod tests {
    ...
    async fn test_get_model_ids_format() {
        let service = ModelDiscoveryService::default();
        service.initialize().await;
        // Mock registry or just call it with empty
        let registry = crate::provider::ProviderRegistry::new();
        service.refresh(&registry).await;
        let ids = service.get_model_ids().await;
        // On CI/test it might be empty if no providers, but should contain / if not empty
        for id in ids {
            assert!(id.contains('/'));
        }
    }
