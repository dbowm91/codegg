...
impl SemanticContextCollector {
    ...
    pub async fn collect(
        ...
        // would service this file has been created and is in a
        // non-usable state, short-circuit with a structured partial
        // response. A `None` readiness (no client has been created
        // yet) does NOT short-circuit — the request phases below
        // lazily create the client via `get_or_create_client`.
        // A `Failed` state propagates as an `Err` so the agent
        // sees the failure clearly (mirrors the Phase 0 contract).
        let readiness = self.service.operational_state_for_file(&file).await;
        if let Some(state) = readiness.as_ref() {
            if let LspOperationalState::Failed { reason } = state {
                return Err(format!("LSP server failed: {reason}"));
            }
            if !state.is_usable() {
