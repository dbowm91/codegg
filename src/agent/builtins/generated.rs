// AUTO-GENERATED by scripts/generate_builtin_agents.py — DO NOT EDIT

use crate::agent::{Agent, AgentMode, AgentRuntimeKind};
use std::collections::HashMap;

pub fn generated_builtin_agents() -> Vec<Agent> {
    vec![
        Agent {
            name: "build".to_string(),
            role: Some("executor".to_string()),
            description: "Default agent with full permissions".to_string(),
            mode: AgentMode::Primary,
            mode_name: None,
            model: None,
            fallback_model: None,
            variant: None,
            temperature: None,
            top_p: None,
            color: None,
            steps: None,
            system_prompt: None,
            permissions: HashMap::new(),
            hidden: false,
            thinking_budget: None,
            reasoning_effort: None,
            runtime_kind: None,
        },
        Agent {
            name: "compaction".to_string(),
            role: Some("compactor".to_string()),
            description: "Context compaction agent".to_string(),
            mode: AgentMode::Subagent,
            mode_name: None,
            model: None,
            fallback_model: None,
            variant: None,
            temperature: None,
            top_p: None,
            color: None,
            steps: None,
            system_prompt: Some("You are a context compaction agent. You summarize conversation history to fit within context limits.\n\nFocus on:\n- Preserving key decisions and their rationale\n- Maintaining file references and line numbers\n- Keeping active task state and blockers\n- Condensing tool outputs to essential information\n\nDrop resolved discussions, verbose tool output, and stale exploration. Preserve the signal, discard the noise.".to_string()),
            permissions: HashMap::from([
                ("*".to_string(), "deny".to_string()),
            ]),
            hidden: true,
            thinking_budget: None,
            reasoning_effort: None,
            runtime_kind: Some(AgentRuntimeKind::Compaction),
        },
        Agent {
            name: "explore".to_string(),
            role: Some("explorer".to_string()),
            description: "Read-only exploration agent".to_string(),
            mode: AgentMode::All,
            mode_name: None,
            model: None,
            fallback_model: None,
            variant: None,
            temperature: None,
            top_p: None,
            color: None,
            steps: None,
            system_prompt: Some("You are a read-only exploration agent. You search and analyze code without making any changes.\n\nFocus on:\n- Finding relevant files, functions, and types\n- Understanding code structure and relationships\n- Answering questions about codebase behavior\n- Tracing execution paths and data flow\n\nFor systematic exploration across many files, prefer `tool_program`\nover sequential direct calls. Use `tool_program` with `read`, `glob`,\n`grep`, and `list` to batch parallel reads, filter results, and\naggregate findings in a single program. Use direct tools when you\nneed to inspect a specific output and reason about it before deciding\nthe next step.\n\nDo not edit files, run shell commands, or modify any state. Report findings clearly.".to_string()),
            permissions: HashMap::from([
                ("apply_patch".to_string(), "deny".to_string()),
                ("bash".to_string(), "deny".to_string()),
                ("commit".to_string(), "deny".to_string()),
                ("edit".to_string(), "deny".to_string()),
                ("image".to_string(), "deny".to_string()),
                ("lsp_preview_apply".to_string(), "deny".to_string()),
                ("plan_enter".to_string(), "deny".to_string()),
                ("plan_exit".to_string(), "deny".to_string()),
                ("replace".to_string(), "deny".to_string()),
                ("terminal".to_string(), "deny".to_string()),
                ("todoread".to_string(), "deny".to_string()),
                ("todowrite".to_string(), "deny".to_string()),
                ("write".to_string(), "deny".to_string()),
            ]),
            hidden: false,
            thinking_budget: None,
            reasoning_effort: None,
            runtime_kind: None,
        },
        Agent {
            name: "general".to_string(),
            role: Some("executor".to_string()),
            description: "Subagent without todo/goal management".to_string(),
            mode: AgentMode::Subagent,
            mode_name: None,
            model: None,
            fallback_model: None,
            variant: None,
            temperature: None,
            top_p: None,
            color: None,
            steps: None,
            system_prompt: Some("You are a general-purpose subagent. You perform tasks delegated by the primary agent.\n\nFocus on:\n- Completing the assigned task efficiently\n- Using only the tools available to you\n- Reporting results clearly back to the calling agent\n\nYou do not have access to todo or goal management tools. Focus on execution.\n\n## Isolated delegated runs\n\nMutating durable runs execute in a CodeGG-managed worktree whose path is the\nonly writable repository scope. Keep all edits, staging, and commits inside\nthat worktree. The child result is machine-derived from Git state; your prose\nsummary is explanatory only. Completion never changes the parent repository.\nDo not push, rewrite history, reset, clean, change remotes/configuration, or\nattempt to integrate into the parent. Report conflicts without discarding the\nworktree so the parent can choose an explicit typed integration or recovery.\n\n## Tool Programs\n\nFor tasks requiring 3+ read-only tool calls with deterministic logic\n(filtering, aggregation, formatting), prefer `tool_program` over\nsequential direct calls. Use `tool_program` with `read`, `glob`,\n`grep`, and `list` for batch parallel reads and mechanical processing.\n\nUse direct tools when the task requires semantic reasoning about each\nresult or when mutation is needed. Intermediate program outputs stay\nin the artifact ledger and do not enter the transcript.\n\n## Git workflow guidance (Phase F)\n\nWhen working with Git repositories, prefer the typed `git` tool surface\nover raw Bash `git` invocations:\n\n- Use read-only subcommands (`status`, `diff`, `log`, `show`, `blame`) directly — they return structured JSON via `egggit`.\n- Use the `mutation` action (e.g. `stage_paths`, `commit`, `branch_create`, `merge`, `revert`, `push`, `reset_hard`, `clean`) for state-changing operations. Avoid `git add -A` / `git add .` unless explicitly intended; stage explicit paths.\n- Inspect state before destructive operations: query `operation_state` first to confirm the active operation, conflicted paths, and legal recovery actions.\n- For in-progress operations, use `recover` (`continue` | `abort` | `skip`) — these are operation-aware and refuse cross-operation misuse (e.g. `rebase --abort` while a merge is in progress).\n- Conflicts are NOT auto-resolved. Edit conflict markers in the worktree, stage resolutions with `git add <path>` (use the `stage_paths` mutation), then `recover: continue`.\n- Avoid destructive cleanup (force pushes, `reset --hard`, `clean -f`) without explicit user authorization. The typed mutation API surfaces these as destructive; the permission flow will gate them accordingly.\n- Use the dedicated commit/subagent tooling (or `mutation: \"commit\"` with an explicit `message`) for message generation. Inline shell pipelines like `git commit -m \"...\"` are translated by Bash routing but lack the structured snapshot/delta/RunStore integration.".to_string()),
            permissions: HashMap::from([
                ("plan_enter".to_string(), "deny".to_string()),
                ("plan_exit".to_string(), "deny".to_string()),
                ("task".to_string(), "allow".to_string()),
                ("todoread".to_string(), "deny".to_string()),
                ("todowrite".to_string(), "deny".to_string()),
            ]),
            hidden: false,
            thinking_budget: None,
            reasoning_effort: None,
            runtime_kind: None,
        },
        Agent {
            name: "plan".to_string(),
            role: Some("planner".to_string()),
            description: "Read-only agent for planning".to_string(),
            mode: AgentMode::Primary,
            mode_name: None,
            model: None,
            fallback_model: None,
            variant: None,
            temperature: None,
            top_p: None,
            color: None,
            steps: None,
            system_prompt: Some("You are a planning agent. You analyze tasks and create structured plans without making any changes to the codebase.\n\nFocus on:\n- Understanding the task requirements\n- Identifying files and functions that need changes\n- Breaking work into concrete, ordered steps\n- Estimating complexity and risk\n\nDo not execute code, edit files, or run shell commands. Your output is a plan for the primary agent to execute.".to_string()),
            permissions: HashMap::from([
                ("apply_patch".to_string(), "deny".to_string()),
                ("bash".to_string(), "deny".to_string()),
                ("commit".to_string(), "deny".to_string()),
                ("edit".to_string(), "deny".to_string()),
                ("lsp_preview_apply".to_string(), "deny".to_string()),
                ("replace".to_string(), "deny".to_string()),
                ("terminal".to_string(), "deny".to_string()),
                ("write".to_string(), "deny".to_string()),
            ]),
            hidden: false,
            thinking_budget: None,
            reasoning_effort: None,
            runtime_kind: None,
        },
        Agent {
            name: "research".to_string(),
            role: Some("researcher".to_string()),
            description: "Long-horizon research agent. Uses the `research` tool for structured multi-source answers and the `websearch` tool for quick lookups.".to_string(),
            mode: AgentMode::All,
            mode_name: None,
            model: None,
            fallback_model: None,
            variant: None,
            temperature: None,
            top_p: None,
            color: Some("magenta".to_string()),
            steps: None,
            system_prompt: Some("You are codegg's research agent. You produce long-horizon, multi-source answers.\nFor in-depth, comparative, or multi-hop questions, prefer the `research` tool — it runs the full pipeline (source collection, evidence extraction, claim construction, citation verification, synthesis).\nFor a quick lookup, use the `websearch` tool directly (defaults to DuckDuckGo, no key required; falls back to Mojeek; uses key-based providers if their env vars are set).\nAlways cite sources in your final output. When the `research` tool is available, prefer it for synthesis; `websearch` is for lookups.\nAvoid `curl`/`wget` for web search — use the `websearch` tool.".to_string()),
            permissions: HashMap::from([
                ("apply_patch".to_string(), "deny".to_string()),
                ("bash".to_string(), "deny".to_string()),
                ("codesearch".to_string(), "allow".to_string()),
                ("commit".to_string(), "deny".to_string()),
                ("edit".to_string(), "deny".to_string()),
                ("glob".to_string(), "allow".to_string()),
                ("grep".to_string(), "allow".to_string()),
                ("image".to_string(), "deny".to_string()),
                ("list".to_string(), "allow".to_string()),
                ("lsp_preview_apply".to_string(), "deny".to_string()),
                ("plan_enter".to_string(), "deny".to_string()),
                ("plan_exit".to_string(), "deny".to_string()),
                ("question".to_string(), "allow".to_string()),
                ("read".to_string(), "allow".to_string()),
                ("research".to_string(), "allow".to_string()),
                ("skill".to_string(), "allow".to_string()),
                ("task".to_string(), "allow".to_string()),
                ("terminal".to_string(), "deny".to_string()),
                ("todoread".to_string(), "allow".to_string()),
                ("todowrite".to_string(), "allow".to_string()),
                ("webfetch".to_string(), "allow".to_string()),
                ("websearch".to_string(), "allow".to_string()),
                ("write".to_string(), "deny".to_string()),
            ]),
            hidden: false,
            thinking_budget: None,
            reasoning_effort: None,
            runtime_kind: Some(AgentRuntimeKind::Research),
        },
        Agent {
            name: "security-review".to_string(),
            role: Some("security_reviewer".to_string()),
            description: "Defensive security review of changed code. Uses securityContext and deterministic scanning to produce evidence-based findings.".to_string(),
            mode: AgentMode::Subagent,
            mode_name: None,
            model: None,
            fallback_model: None,
            variant: None,
            temperature: None,
            top_p: None,
            color: None,
            steps: None,
            system_prompt: Some("You are a defensive code security reviewer. Use the `security` tool for deterministic scanning and the `lsp` tool (securityContext operation) for risk-marker evidence around changed code.\n\nWorkflow:\n1. Identify changed files and hunks (use git diff or the security workflow).\n2. Classify each file into a security preset (rust_server, rust_cli, web_backend, dependency_review, unsafe_review).\n3. Run deterministic preflight checks (secret/unsafe pattern scans) on changed lines.\n4. Request securityContext around changed hunks and high-risk symbols.\n5. Correlate risk markers, diagnostics, symbols, and call expansion.\n6. Produce findings only when there is concrete evidence.\n7. Distinguish review prompts (marker-only) from confirmed findings.\n8. Suggest minimal mitigations or tests.\n\nRisk markers are review prompts, not findings. Never emit a finding from a marker alone.\nDo not provide exploit steps or offensive automation. Never mutate files during review.".to_string()),
            permissions: HashMap::from([
                ("apply_patch".to_string(), "deny".to_string()),
                ("bash".to_string(), "ask".to_string()),
                ("commit".to_string(), "deny".to_string()),
                ("edit".to_string(), "deny".to_string()),
                ("glob".to_string(), "allow".to_string()),
                ("grep".to_string(), "allow".to_string()),
                ("image".to_string(), "deny".to_string()),
                ("list".to_string(), "allow".to_string()),
                ("lsp".to_string(), "allow".to_string()),
                ("lsp_preview_apply".to_string(), "deny".to_string()),
                ("plan_enter".to_string(), "deny".to_string()),
                ("plan_exit".to_string(), "deny".to_string()),
                ("read".to_string(), "allow".to_string()),
                ("replace".to_string(), "deny".to_string()),
                ("security".to_string(), "allow".to_string()),
                ("terminal".to_string(), "deny".to_string()),
                ("todoread".to_string(), "deny".to_string()),
                ("todowrite".to_string(), "deny".to_string()),
                ("write".to_string(), "deny".to_string()),
            ]),
            hidden: false,
            thinking_budget: None,
            reasoning_effort: None,
            runtime_kind: Some(AgentRuntimeKind::SecurityReview),
        },
        Agent {
            name: "summary".to_string(),
            role: Some("summarizer".to_string()),
            description: "Generates session summaries".to_string(),
            mode: AgentMode::Subagent,
            mode_name: None,
            model: None,
            fallback_model: None,
            variant: None,
            temperature: None,
            top_p: None,
            color: None,
            steps: None,
            system_prompt: None,
            permissions: HashMap::from([
                ("plan_enter".to_string(), "deny".to_string()),
                ("plan_exit".to_string(), "deny".to_string()),
                ("todoread".to_string(), "deny".to_string()),
                ("todowrite".to_string(), "deny".to_string()),
            ]),
            hidden: true,
            thinking_budget: None,
            reasoning_effort: None,
            runtime_kind: Some(AgentRuntimeKind::Summary),
        },
        Agent {
            name: "title".to_string(),
            role: Some("title".to_string()),
            description: "Generates session titles".to_string(),
            mode: AgentMode::Subagent,
            mode_name: None,
            model: None,
            fallback_model: None,
            variant: None,
            temperature: None,
            top_p: None,
            color: None,
            steps: None,
            system_prompt: None,
            permissions: HashMap::from([
                ("plan_enter".to_string(), "deny".to_string()),
                ("plan_exit".to_string(), "deny".to_string()),
                ("todoread".to_string(), "deny".to_string()),
                ("todowrite".to_string(), "deny".to_string()),
            ]),
            hidden: true,
            thinking_budget: None,
            reasoning_effort: None,
            runtime_kind: Some(AgentRuntimeKind::Title),
        },
        Agent {
            name: "verifier".to_string(),
            role: Some("semantic_verifier".to_string()),
            description: "Independent, read-only semantic verification of a producer result.".to_string(),
            mode: AgentMode::Subagent,
            mode_name: None,
            model: None,
            fallback_model: None,
            variant: None,
            temperature: None,
            top_p: None,
            color: None,
            steps: None,
            system_prompt: Some("You are an independent semantic verifier. The producer and verifier are\nseparate roles: challenge the producer's assumptions and inspect the bounded\nhost evidence supplied in your prompt. Treat only that evidence and your\nread-only observations as facts. Do not claim to have run checks that are not\nrecorded in the evidence.\n\nReturn exactly one JSON object wrapped in the required\n`<convergence_verdict>...</convergence_verdict>` marker. The JSON must have\n`kind` (`pass`, `revise`, or `inconclusive`), a concise `summary`, and an\n`evidence_refs` array. Use `inconclusive` whenever required evidence is absent\nor uncertainty remains. A semantic pass is not goal completion, merge\napproval, permission approval, or authorization to mutate anything.".to_string()),
            permissions: HashMap::from([
                ("apply_patch".to_string(), "deny".to_string()),
                ("bash".to_string(), "deny".to_string()),
                ("commit".to_string(), "deny".to_string()),
                ("diff".to_string(), "allow".to_string()),
                ("edit".to_string(), "deny".to_string()),
                ("evidence_bundle".to_string(), "allow".to_string()),
                ("git".to_string(), "deny".to_string()),
                ("glob".to_string(), "allow".to_string()),
                ("goal_get".to_string(), "deny".to_string()),
                ("goal_request_completion".to_string(), "deny".to_string()),
                ("goal_update_progress".to_string(), "deny".to_string()),
                ("grep".to_string(), "allow".to_string()),
                ("list".to_string(), "allow".to_string()),
                ("lsp".to_string(), "allow".to_string()),
                ("lsp_preview_apply".to_string(), "deny".to_string()),
                ("permission".to_string(), "deny".to_string()),
                ("python".to_string(), "deny".to_string()),
                ("python_script".to_string(), "deny".to_string()),
                ("question".to_string(), "deny".to_string()),
                ("read".to_string(), "allow".to_string()),
                ("replace".to_string(), "deny".to_string()),
                ("repo_fetch".to_string(), "deny".to_string()),
                ("research".to_string(), "deny".to_string()),
                ("task".to_string(), "deny".to_string()),
                ("terminal".to_string(), "deny".to_string()),
                ("test".to_string(), "deny".to_string()),
                ("webfetch".to_string(), "deny".to_string()),
                ("websearch".to_string(), "deny".to_string()),
                ("write".to_string(), "deny".to_string()),
            ]),
            hidden: true,
            thinking_budget: None,
            reasoning_effort: None,
            runtime_kind: None,
        },
    ]
}
