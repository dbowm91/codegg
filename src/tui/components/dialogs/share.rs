...
impl ShareDialog {
    ...
    fn paragraph(&self, theme: &Theme) -> Paragraph<'static> {
        ...
        Paragraph::new(self.build_lines(theme))
            .block(block)
            .wrap(Wrap { trim: true })
    }
}
...
impl Widget for &ShareDialog {
    fn render(self, area: Rect, buf: &mut ratatui::prelude::Buffer) {
        self.paragraph(&self.theme).render(area, buf);
    }
}
...
impl Component for ShareDialog {
    ...
    fn render(&mut self, frame: &mut Frame, area: Rect, theme: &Arc<Theme>) {
        self.paragraph(theme).render(area, frame.buffer_mut());
    }
...
mod tests {
    ...
    #[test]
    fn widget_and_component_render_paths_are_equivalent() {
        for (url, copied) in [("", false), ("https://example.com", true)] {
            let theme = Arc::new(Theme::dark());
            let mut widget_dialog = ShareDialog::new(Arc::clone(&theme));
            widget_dialog.set_url(url.to_string());
            widget_dialog.copied = copied;

            let mut widget_terminal = Terminal::new(TestBackend::new(80, 40)).unwrap();
            widget_terminal
                .draw(|frame| frame.render_widget(&widget_dialog, frame.area()))
                .unwrap();

            let mut component_dialog = widget_dialog.clone();
            let mut component_terminal = Terminal::new(TestBackend::new(80, 40)).unwrap();
            component_terminal
                .draw(|frame| {
                    let area = frame.area();
                    Component::render(&mut component_dialog, frame, area, &theme);
                })
                .unwrap();

            assert_eq!(
                widget_terminal.backend().buffer(),
                component_terminal.backend().buffer(),
                "render paths diverged for url={url:?} copied={copied}"
            );
        }
    }
}
